...
import { User } from '@/types/User' // Adjust import as needed

interface Props {
    loggedUser: User
}

const DropdownUser = ({ loggedUser }: Props) => {
    ...
    const dropdownClasses = `absolute right-0 mt-4 flex w-62.5 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark ${dropdownOpen ? 'block' : 'hidden'}`
    ...
    return (
        <div className="relative">
            ...
            <div
                ...
                <button
                    onClick={logout}
                    className="px-6 py-3 text-sm font-medium text-red-500 hover:text-red-600">
                    Logout
                </button>
            </div>
        </div>
